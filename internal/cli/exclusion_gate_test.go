...
func TestExclusionHeldByGateDespiteConfig(t *testing.T) {
	...
	got := skillCoveredExclusions(injectConfigOnly(nil, monoRoot), projectID, layout)
	if len(got) != 0 {
...
func TestDeployedButUnusedPortfolioDoesNotOpenTheGate(t *testing.T) {
	...
	if got := skillCoveredExclusions(injectConfigOnly(nil, monoRoot), projectID, layout); len(got) != 0 {
